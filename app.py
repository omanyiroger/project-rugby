...
def login():
    if request.method == "POST":
        ...
        user = conn.execute("SELECT * FROM user_roles WHERE username = ?", (username,)).fetchone()
        conn.close()

        if user and check_password_hash(user["password"], password):
            session["user"] = username
            session["role"] = user["role"]  # store role for access control later
            return redirect(url_for("dashboard"))
...
def view_availability():
    if "user" not in session:
        flash("You must be logged in to view availability.")
        return redirect(url_for('login'))
    players = []
    day = None
    conn = get_db_connection()
...
def view_strapping():
    if "user" not in session:
        flash("You must be logged in to view strapping information.")
        return redirect(url_for('login'))
    conn = get_db_connection()
    players = conn.execute("""
...
def view_first_team():
    if "user" not in session:
        flash("You must be logged in to view the second team.")
        return redirect(url_for("login"))
    conn = get_db_connection()
    players = conn.execute("""
        SELECT p.player_id, p.name, p.position
...
def insert_sample_users():
    # Only run once, then skip future requests
    if not hasattr(app, 'sample_users_added'):
        conn = sqlite3.connect('rcmis.db')
        cursor = conn.cursor()
        users = [
